...
Seninle birlikte **`+üyesayısı+`** kişiye ulaştık ! Tagımızı (\`${settings.taglar.servertag}\`) alarak bizlere destek olabilirsin ! Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor yetkililerimiz <@&799708207253291058> seninle ilgilenecektir. İyi eğlenceler.`)});
...
client.on("guildMemberAdd", member => {
  ...
  let channel = client.guilds.cache.get(sunucuid).channels.cache.find(x => x.name == 'auto'); //tagrol-log yerine kendi log kanalınızın ismini yazabilirsiniz
